#!/usr/bin/env python3
"""
Season 0 role sync utility.

What it does:
1) Remove "Membro" role from all members who have it.
2) Remove Season 0 roles from all members who have them.
3) Read legacy DB ranking and assign:
   - TOP 1 / TOP 2 / TOP 3
...
ROLE_NAME_DEFAULTS = {
    "member": "Membro",
    "top1": "\U0001F947TOP 1 - SEASON 0",
    "top2": "\U0001F948TOP 2 - SEASON 0",
...
ROLE_ENV_OVERRIDES = {
    ...
    "top1": "ROLE_TOP1_SEASON0_ID",
    "top2": "ROLE_TOP2_SEASON0_ID",
    "top3": "ROLE_TOP3_SEASON0_ID",
    "top10": "ROLE_TOP10_SEASON0_ID",
    "top50": "ROLE_TOP50_SEASON0_ID",
    "founder": "ROLE_FOUNDER_SEASON0_ID",
}
        ...
        self.session.headers.update(
            {
                ...
                "User-Agent": "mixbot-season0-role-sync/1.0",
            }
        )
...
def resolve_role_ids(dc: DiscordClient, guild_id: str, roles: List[Dict], create_missing: bool) -> Dict[str, str]:
    ...
    for key, role_name in ROLE_NAME_DEFAULTS.items():
        ...
        if role_id.isdigit():
            ...
            continue
        # Optional auto-create for season roles.
        if create_missing and key != "member":
            created = dc.request(
                "POST",
...
def read_legacy_ranking_rows() -> List[Tuple[str, int, int, int]]:
    cfg = {
        "host": env("LEGADO_DB_HOST"),
        "user": env("LEGADO_DB_USER"),
        "password": env("LEGADO_DB_PASSWORD"),
        "database": env("LEGADO_DB_NAME"),
        "port": int(env("LEGADO_DB_PORT", "3306")),
        "charset": "utf8mb4",
    }
    ...
    try:
        with conn.cursor() as c:
            c.execute(
                ...
                ORDER BY rating DESC, total_matches DESC, wins DESC, CAST(discord_id AS UNSIGNED) ASC
                """
            )
            rows = c.fetchall()
    finally:
    ...
    clean: List[Tuple[str, int, int, int]] = []
    seen: Set[str] = set()
    for discord_id, total_matches, rating, wins in rows:
        ...
        if did in seen:
            continue
        seen.add(did)
        clean.append((did, int(total_matches or 0), int(rating or 0), int(wins or 0)))
    return clean


def build_cohorts(rows: List[Tuple[str, int, int, int]]) -> Dict[str, List[str]]:
    ...
    cohorts = {
        ...
        "top10": ids[3:10],
        "top50": ids[10:50],
        "founder": [did for did, total_matches, _rating, _wins in rows if total_matches > 10],
    }
    return cohorts
...
def apply_or_preview(
    ...
    season_role_keys = ["top1", "top2", "top3", "top10", "top50", "founder"]
    reset_role_keys = ["member"] + season_role_keys
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sync Season 0 Discord roles from legacy ranking.")
    ...
    rows = read_legacy_ranking_rows()
    cohorts = build_cohorts(rows)

    mode = "APPLY" if args.apply else "DRY-RUN"
    print(f"Mode: {mode}")
    apply_or_preview(dc, guild_id, role_ids, cohorts, apply=bool(args.apply))
    print("Done.")
    return 0
